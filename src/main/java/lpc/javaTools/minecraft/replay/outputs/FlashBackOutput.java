...
public class FlashBackOutput implements Output {
	...
	private static JsonObject kfCameraJson(KeyFrame kf) {
		...
		//camera.position.add(camera.getForwardVector().mul(20));
		position.add(camera.position.x);
		position.add(camera.position.y);
		position.add(camera.position.z);
		kfJson.add("position", position);
		Vector3d ypr = kf.camera().getYawPitchRoll();
		kfJson.addProperty("yaw", Math.toDegrees(ypr.x));
		kfJson.addProperty("pitch", Math.toDegrees(ypr.y));
		kfJson.addProperty("roll", Math.toDegrees(ypr.z));
